...
#define USB_FRAME_BYTES   (BUFF_LEN_1 * 3)

static uint8_t usbBuf[USB_FRAME_BYTES];
...
int main(void)
{
    ...
    */
    ...
    while (1)
		{
				/* Wait for a completed PDMA audio frame */
			printf("DTR = %d\n", (g_u16CtrlSignal & 1));
			CLK_SysTickDelay(1000);
				...
				/* Safely grab the completed buffer */
				__disable_irq();
				uint8_t buf = g_RxBufIdx ^ 1u;
				g_NewFrame = 0;
				__enable_irq();

				/* Pack 24-bit PCM samples into USB buffer */
				uint32_t idx = 0;
				...
				/* Wait until previous USB IN transfer completes */
				while (g_u32TxSize)
				{
						if (g_u8Suspend)   // USB suspended / unplugged
								break;
				}

				/* Send only if USB is active */
				if (!g_u8Suspend && idx)
				{
						USBD_MemCopy(
								(uint8_t *)(USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP2)),
